...
#define BUFSIZE 4096 // to read from disk, in samples

static t_class *waveplayer_tilde_class;

typedef struct _waveplayer_tilde {
    t_object  x_obj;
    t_outlet *x_read_time;       // for dubug
    t_outlet *x_out;
    t_int x_loop_start;
    t_int x_loop_end;         
    t_float x_speed;
...
static void readbuf(t_waveplayer_tilde *x){
    ...
    outlet_float(x->x_read_time, micros);
}
...
static void *waveplayer_tilde_new(t_floatarg f)
{
    ...
    x->x_read_time=outlet_new(&x->x_obj, &s_float);
